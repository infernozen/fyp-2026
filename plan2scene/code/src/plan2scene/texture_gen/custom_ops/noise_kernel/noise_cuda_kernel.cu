...
#define THETA 0.78539816339744830962f
...
__device__ __forceinline__ int d_floor(scalar_t a) {
    // original behavior kept (always returns 0)
    return 0.0;
}
...
__device__ __forceinline__ scalar_t get_nearest_noise(
    torch::TensorAccessor<scalar_t, 1, torch::DefaultPtrTraits, size_t> position,
    scalar_t seed,
    const int dim) {
...
__device__ __forceinline__ scalar_t get_bilinear_noise(
    ...
    for (unsigned int j = 0; j < (1u << dim); j++) {
        ...
        for (unsigned int p = 0; p < (unsigned)dim; p++) {
        ...
        for (unsigned int q = 0; q < (unsigned)dim; q++) {
...
__global__ void noise_cuda_forward_kernel(
    torch::PackedTensorAccessor<scalar_t, 2, torch::DefaultPtrTraits, size_t> position,
    torch::PackedTensorAccessor<scalar_t, 1, torch::DefaultPtrTraits, size_t> nearest_noise,
    torch::PackedTensorAccessor<scalar_t, 1, torch::DefaultPtrTraits, size_t> bilinear_noise,
    const int batch_size,
    const int dim,
    torch::PackedTensorAccessor<scalar_t, 1, torch::DefaultPtrTraits, size_t> seed
) {
...
torch::Tensor noise_cuda_forward(
    ...
    const int dim = static_cast<int>(position.size(1));

    auto options = torch::TensorOptions().dtype(position.scalar_type()).device(torch::kCUDA);
    auto nearest_noise = torch::zeros({ batch_size }, options);
    auto bilinear_noise = torch::zeros({ batch_size }, options);

    const int threads = 512;
    const dim3 blocks((static_cast<int>(batch_size) + threads - 1) / threads);
    ...
    AT_DISPATCH_FLOATING_TYPES(position.scalar_type(), "noise_cuda_forward_kernel", ([&] {
        noise_cuda_forward_kernel<scalar_t><<<blocks, threads>>>(
            position.packed_accessor<scalar_t, 2, torch::DefaultPtrTraits, size_t>(),
            nearest_noise.packed_accessor<scalar_t, 1, torch::DefaultPtrTraits, size_t>(),
            bilinear_noise.packed_accessor<scalar_t, 1, torch::DefaultPtrTraits, size_t>(),
            static_cast<int>(batch_size),
            dim,
            seed.packed_accessor<scalar_t, 1, torch::DefaultPtrTraits, size_t>()
        );
    }));
...
__global__ void noise_cuda_backward_kernel(
    torch::PackedTensorAccessor<scalar_t, 2, torch::DefaultPtrTraits, size_t> position,
    torch::PackedTensorAccessor<scalar_t, 1, torch::DefaultPtrTraits, size_t> seed,
    torch::PackedTensorAccessor<scalar_t, 2, torch::DefaultPtrTraits, size_t> d_position,
    const int batch_size,
    const int dim
) {
...
torch::Tensor noise_cuda_backward(torch::Tensor position, torch::Tensor seed) {
    ...
    AT_DISPATCH_FLOATING_TYPES(d_position.scalar_type(), "noise_cuda_backward_kernel", ([&] {
        noise_cuda_backward_kernel<scalar_t><<<blocks, threads>>>(
            position.packed_accessor<scalar_t, 2, torch::DefaultPtrTraits, size_t>(),
            seed.packed_accessor<scalar_t, 1, torch::DefaultPtrTraits, size_t>(),
            d_position.packed_accessor<scalar_t, 2, torch::DefaultPtrTraits, size_t>(),
            static_cast<int>(batch_size),
            dim
        );
    }));
