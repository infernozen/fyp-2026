# ==== IP.py FIX (drop-in header + stubs) ====
from pulp import *
import cv2, numpy as np, sys, csv, copy, os
from utils import *
from skimage import measure
from floorplan_utils import calcLineDim
from floorplan_utils import pointDistance,drawPoints, drawPointsSeparately, drawLines, drawRectangles, drawLineMask, drawSegmentationImage, drawResultImage, drawResultImageFinal, resizeResult, writeRepresentation


from train import NUM_ICONS, NUM_ROOMS, NUM_CORNERS
from floorplan_utils import getOrientationRanges, getIconNames,POINT_ORIENTATIONS

# ---- missing globals ----
iconNames = getIconNames()
ORIENTATION_RANGES = getOrientationRanges(256, 256)

# ---- missing API (QP compatibility) ----
def findMatches(*args, **kwargs):
    return {'wall':[0,0,0],'door':[0,0,0],'icon':[0,0,0],'room':[0,0,0]}
...
def findCandidatesFromHeatmaps(iconHeatmaps, iconPointOffset, doorPointOffset):
  ...
  newDoorLines = []
  newDoorPoints = []
  for iconIndex in range(1, min(NUM_ICONS + 2, iconHeatmaps.shape[2])):
...
def reconstructFloorplan(wallCornerHeatmaps, doorCornerHeatmaps, iconCornerHeatmaps, iconHeatmaps, roomHeatmaps, output_prefix='test/', densityImage=None, gt_dict=None, gt=False, gap=-1, distanceThreshold=-1, lengthThreshold=-1, debug_prefix='test', heatmapValueThresholdWall=None, heatmapValueThresholdDoor=None, heatmapValueThresholdIcon=None, enableAugmentation=False):
  ...
  if True:
    ...
    wallLineConfidenceMap = roomHeatmaps.max(axis=2)
    ...
    doorLineConfidenceMap = iconHeatmaps.max(axis=2)
    ...
    for iconIndex, icon in enumerate(icons):
      ...
      for typeIndex in range(len(iconConfidence)):
    ...
    if LpStatus[model.status] == 'Optimal':
      ...
      drawLines('', width, height, wallPoints, filteredWallLines, [], img, lineColor=255)
      ...
      drawLines('', width, height,
          ...
          img,
          lineColor=255)
      cv2.imwrite(str(output_prefix) + 'result_door.png', img)
      ...
      drawRectangles(str(output_prefix) + 'result_icon.png', width, height, iconPoints, filteredIcons, filteredIconTypes)
      ...
      with open(os.path.join(str(debug_prefix), 'floorplan.txt'), 'w') as result_file:
